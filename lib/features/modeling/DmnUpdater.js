...
function DmnUpdater(eventBus, moddle, elementRegistry) {
  ...
  function setColumnParent(event) {
    ...
    if(event.context.column.next && event.context.column.next.businessObject) {
    ...
    // add the itemDefinition of the Clause to the ItemDefinitions of the Definitions object
    ...
    if(event.context.column.businessObject.inputExpression) {
      p.ItemDefinition.push(event.context.column.businessObject.inputExpression.itemDefinition.ref);
    } else {
      p.ItemDefinition.push(event.context.column.businessObject.outputDefinition.ref);
    }
