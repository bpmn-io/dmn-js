 ...
 * @param {sheet} sheet
 */
function EditCellHandler(sheet, elementRegistry, graphicsFactory, moddle) {
  this._sheet = sheet;
...
EditCellHandler.$inject = [ 'sheet', 'elementRegistry', 'graphicsFactory', 'moddle' ];
...
EditCellHandler.prototype.execute = function(context) {
  ...
  if(el.row.isHead) {
    console.log(el.column.businessObject);
    ...
    if(el.content) {
      ...
      var newSemantic = this._moddle.create('dmn:LiteralExpression', {
        id: el.id,
        text: context.content
      });
      if(el.column.businessObject.inputEntry) {
        el.column.businessObject.inputEntry.push(newSemantic);
      } else {
        el.column.businessObject.outputEntry.push(newSemantic);
      }
      el.row.businessObject[el.column.businessObject.inputEntry ? 'condition' : 'conclusion'].push(newSemantic);
      el.content = newSemantic;
    }
