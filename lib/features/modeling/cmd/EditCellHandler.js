...
EditCellHandler.prototype.execute = function(context) {
  ...
  if(el.row.isHead) {
    if(el.row.isMappingsRow) {
      ...
      context.oldContent = el.content.output;
      el.content.output = context.content;
    ...
    if(el.column.isAnnotationsColumn) {
      ...
      // remove empty cells
      if(context.content === '') {
        var condOrConc = el.column.businessObject.inputEntry ? 'condition' : 'conclusion';
        ...
        el.row.businessObject[condOrConc].splice(
          el.row.businessObject[condOrConc].indexOf(el.content), 1);
        delete el.content;
      }
    }
  }
...
EditCellHandler.prototype.revert = function(context) {
  ...
  if(el.row.isHead) {
    if(el.row.isMappingsRow) {
      ...
      el.content.output = context.oldContent;
    } else if(el.row.isClauseRow) {
      // revert clause name
