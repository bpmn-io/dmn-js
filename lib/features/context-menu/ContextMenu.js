...
function ContextMenu(popupMenu, eventBus, modeling, elementRegistry) {
...
ContextMenu.$inject = [ 'popupMenu', 'eventBus', 'modeling', 'elementRegistry' ];
...
ContextMenu.prototype.open = function(x, y, context) {
  this._popupMenu.open(
    {
      position: { x: x, y: y },
      entries: this.getActions(context)
    }
  );
};
...
ContextMenu.prototype.ruleClearAction = function(context) {
  var cells = this._elementRegistry.filter(function(ea) {
    return ea.row === context.row && ea.column.id !== 'utilityColumn';
  });
  var self = this;
  forEach(cells, function(cell) {
    self._modeling.editCell(cell.row.id, cell.column.id, '');
  });
  this.close();
};
