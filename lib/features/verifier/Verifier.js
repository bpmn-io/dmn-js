...
Verifier.prototype.verifyTable = function () {
	...
	while(lastRule !== null){
		...
		for(i = 0; i < rule.inputEntry.length; i++){
			...
			domClasses(elReg[cellId].gfx).remove("wrongValue");
		}
		domClasses(elReg[lastRule.id].gfx).remove("overlapping-rules-focused");
		lastRule = lastRule.previous;
	}

	var cellValues = makeCellsWithNewAttributes(this._sheet, this._elementRegistry, ruleOrder);
	
	if(!cellValues[1]){
		var inputAndRules = getRulesAndInputs(cellValues[0], ruleOrder, this._elementRegistry);
		...
		var overlappingRules = findOverlappingRules(rules, ruleArray, 0, inputColumnCount, []);
		outputOverlappingRules(overlappingRules, false, this._elementRegistry, this._modeling, this._sheet);	

		
		var missingRules = findMissingRules(rules, ruleArray, 0, inputColumnCount, []);
...
function makeCellsWithNewAttributes(sheet, elementRegistry, ruleOrder){
	var cellValues = {};
	var hasError = false;
	var inputTypes = getInputTypes(elementRegistry, ruleOrder);
	...
	while(lastRule !== null){
		...
		for(var i = 0; i < ruleInputEntry.length; i++){
			...
			var hasSyntaxError = checkSyntaxErrors(cellId, cellValue, valueType, 
				elementRegistry, sheet.getContainer());
			...
			cellValues[cellId] = new CellAttributes(cellId , valueType,
...
function checkSyntaxErrors(cellId, cellValue, valueType, elementRegistry, container){
	...
	if(cellTrimmedValue !== ""){
		...
		("boolean" === valueType && !(isBoolean(cellTrimmedValue)))){
			addTooltip(cellId, valueType, cellTrimmedValue, elementRegistry, container);
...
function getInputTypes(elementRegistry, columnIds){

	var inputColumnTypes = [];
	for(var i = 0; i < columnIds.length; i++){
		var type = elementRegistry.get(columnIds[i]).businessObject.inputExpression.typeRef;
...
function outputOverlappingRules(overlappingRules, missingRuleCase, elementRegistry, modeling, sheet){
	...
	for(i = 0; i < overlappingRules.length; i++) {
		...
		if(missingRuleCase){
			...
			overlappingRule.addEventListener("click", highlightOverlapingRules(elementRegistry, sheet));
...
function highlightOverlapingRules(elementRegistry, sheet){
	return function (){
...
function CellAttributes(id, type, allowedValue, value, clauseId, ruleId, ruleNR){
	this.type = type;
