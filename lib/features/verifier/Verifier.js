...
Verifier.prototype.verifyTable = function () {
	...
	var errorTableObject = $(this._sheet.getContainer()).get(0).getElementsByClassName("errorTable")[0];
	if(typeof errorTableObject !== undefined){
		$(errorTableObject).remove();
	}
	...
	for(var propertyName in elReg){
		...
		!(propertyName.match(/rule/gi) || propertyName.match(/row-/gi) || 
		propertyName.match(/table-/gi)) && ruleOrder.length === 0){
