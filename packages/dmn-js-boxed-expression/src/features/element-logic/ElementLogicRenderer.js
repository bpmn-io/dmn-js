...
export default class ElementLogicRenderer {
  static $inject = [ 'components', 'viewer' ];

  constructor(components, viewer) {
    components.onGetComponent('body', () => {
      return withChangeSupport(LogicComponent, () => [ viewer.getRootElement() ]);
    });
...
function LogicComponent(_, { injector }) {
  ...
  return (
    <Expression parent={ rootElement } expression={ expression } />
  );
}
