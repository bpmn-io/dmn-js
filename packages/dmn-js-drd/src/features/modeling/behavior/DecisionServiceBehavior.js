...
export default function DecisionServiceBehavior(drdFactory, injector, canvas) {
  ...
  // keep decision service references consistent when a referenced
  // decision or input data is renamed (IdChangeBehavior only updates
  // requirement/association hrefs, not decision service ones)
  this.executed('element.updateProperties', function(event) {
    var context = event.context;

    self._updateReferencesAfterIdChange(context.element, context.oldProperties, context.properties);
  });

  this.reverted('element.updateProperties', function(event) {
    var context = event.context;

    self._updateReferencesAfterIdChange(context.element, context.properties, context.oldProperties);
  });
}
...
/**
 * Keep decision service reference collections (outputDecision,
 * encapsulatedDecision, inputDecision, inputData) in sync when a
 * referenced decision or input data changes id. IdChangeBehavior (shared
 * across all views) only rewrites requirement and association hrefs, not
 * these DRD-specific ones.
 *
 * @param {Element|ModdleElement} element - the renamed element
 * @param {Object} oldProperties
 * @param {Object} properties
 */
DecisionServiceBehavior.prototype._updateReferencesAfterIdChange = function(
    element,
    oldProperties,
    properties
) {
  if (!('id' in oldProperties) || !('id' in properties) || oldProperties.id === properties.id) {
    return;
  }

  var businessObject = getBusinessObject(element);

  if (!is(businessObject, 'dmn:Decision') && !is(businessObject, 'dmn:InputData')) {
    return;
  }

  var oldHref = '#' + oldProperties.id,
      newHref = '#' + properties.id;
  ...
  drgElements.forEach(function(drgElement) {
    ...
    [ 'outputDecision', 'encapsulatedDecision', 'inputDecision', 'inputData' ].forEach(function(name) {
      (drgElement.get(name) || []).forEach(function(ref) {
        if (ref.href === oldHref) {
          ref.href = newHref;
        }
      });
...
DecisionServiceBehavior.prototype.addDecisionToService = function(decisionBo, decisionServiceBo, definitions) {
  ...
  reference.$parent = decisionServiceBo;
...
/**
 * Update which section a decision belongs to, moving it into the given
 * decision service if needed. Handles both repositioning within the same
 * decision service and moving in from a different one, cleaning up any
 * stale reference left behind in the latter case.
 *
...
DecisionServiceBehavior.prototype.updateDecisionSection = function(decisionElement, decisionServiceBo) {
  ...
  var isOutputDecision = this.isOutputDecision(decisionElement, decisionServiceBo);
  var targetCollection = isOutputDecision ? 'outputDecision' : 'encapsulatedDecision';

  var decisionHref = '#' + decisionBo.id;

  var alreadyInTargetSection = (decisionServiceBo.get(targetCollection) || []).some(function(ref) {
    return ref.href === decisionHref;
  });

  if (alreadyInTargetSection) {
    return;
  }

  // Remove any stale reference, whether from this decision service's other
  // section or from a different decision service the decision was moved in
  // from
  var definitions = this._canvas.getRootElement().businessObject;

  this.removeElementFromAllServices(decisionBo.id, definitions);
  ...
  if (!decisionServiceBo[targetCollection]) {
    decisionServiceBo[targetCollection] = [];
  }
  collectionAdd(decisionServiceBo[targetCollection], reference);
