...
const canvas = {
  scrollToElement: function() {
    console.log('scroll to', arguments);
  }
};
 ...
 */
export default function SearchPad(renderer, eventBus, selection) {
  this._open = false;
  this._results = [];
  this._eventMaps = [];
  ...
  this._resultsContainer = domQuery(
    SearchPad.RESULTS_CONTAINER_SELECTOR, this._container);

  // attach search pad
  this._renderer.getContainer().appendChild(this._container);

  // cleanup on destroy
  eventBus.on([ 'table.destroy', 'diagram.destroy' ], this.close, this);
}
...
function createHtmlText(tokens) {
  ...
  tokens.forEach(function(t) {
    if (t.matched) {
      htmlText += '<strong class="' + SearchPad.RESULT_HIGHLIGHT_CLASS + '">' +
      escapeHTML(t.matched) + '</strong>';
    } else {
