...
describe('features/verifier', function() {
    ...
    describe('overlapping rules and overlap', function(verifier) {
        
        it('return array should be empty if no overlap between rules', inject(function(verifier) {
            var ruleOrder = verifier.getInputIds();
            var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
            var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
            var rules = inputAndRules[1];
            ...
            var overlappingRules = verifier.findOverlappingRules(rules, ruleArray, 0, inputColumnCount, []);
            expect(overlappingRules.length === 0).to.be.true;
        }));
        
        it('should return rules which overlap and their overlap (string)', inject(function(verifier, modeling) {
            modeling.createRow({id: 'rule3'});
            modeling.editCell('rule3', 'input1', '"gold"');
            var ruleOrder = verifier.getInputIds();
            var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
            var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
            var rules = inputAndRules[1];
            ...
            var overlappingRules = verifier.findOverlappingRules(rules, ruleArray, 0, inputColumnCount, []);
            var overlappingRuleIDs = Object.keys(overlappingRules[0]);
            expect(overlappingRules.length === 1).to.be.true;
            expect(overlappingRuleIDs.indexOf('rule1') > -1).to.be.true;
            expect(overlappingRuleIDs.indexOf('rule2') > -1).to.be.false;
            expect(overlappingRuleIDs.indexOf('rule3') > -1).to.be.true;
            expect(overlappingRules[0].ruleValue[0] === '"gold"').to.be.true;
            
        }));
        
        it('should return rules which overlap and their overlap. Overlap between two inequalities', inject(function(verifier, modeling, elementRegistry) {
            elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
            modeling.createRow({id: 'rule3'});
            modeling.editCell('rule1', 'input1', '< 100');
            modeling.editCell('rule2', 'input1', '< 1000');
            modeling.editCell('rule3', 'input1', '> 2000');
            var ruleOrder = verifier.getInputIds();
            var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
            var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
            var rules = inputAndRules[1];
            ...
            expect(overlappingRules.length === 1).to.be.true;
            expect(overlappingRuleIDs.indexOf('rule1') > -1).to.be.true;
            expect(overlappingRuleIDs.indexOf('rule2') > -1).to.be.true;
            expect(overlappingRuleIDs.indexOf('rule3') > -1).to.be.false;
            expect(overlappingRules[0].ruleValue[0] === '< 100').to.be.true;
            
        }));
        
        describe('between inequality and interval', function(verifier) {
            it('if one rule overlaps with another rule then returns merged value', inject(function(verifier, modeling, elementRegistry) {
                elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
                modeling.createRow({id: 'rule3'});
                modeling.editCell('rule1', 'input1', '< 100');
                modeling.editCell('rule2', 'input1', '[10, 500)');
                modeling.editCell('rule3', 'input1', '> 2000');
                var ruleOrder = verifier.getInputIds();
                var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
                var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
                ...
                expect(overlappingRuleIDs.indexOf('rule2') > -1).to.be.true;
                expect(overlappingRuleIDs.indexOf('rule3') > -1).to.be.false;
                expect(overlappingRules[0].ruleValue[0] === '[10, 100)').to.be.true;
            }));
            
            it('if one rule during another rule then returns merged value', inject(function(verifier, modeling, elementRegistry) {
                elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
                modeling.createRow({id: 'rule3'});
                modeling.editCell('rule1', 'input1', '< 1000');
                modeling.editCell('rule2', 'input1', '[10, 500]');
                modeling.editCell('rule3', 'input1', '> 2000');
                var ruleOrder = verifier.getInputIds();
                var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
                ...
                expect(overlappingRuleIDs.indexOf('rule2') > -1).to.be.true;
                expect(overlappingRuleIDs.indexOf('rule3') > -1).to.be.false;
                expect(overlappingRules[0].ruleValue[0] === '[10, 500]').to.be.true;
            }));
            
            it('if one rule starts with another rule then returns merged value', inject(function(verifier, modeling, elementRegistry) {
                elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
                modeling.createRow({id: 'rule3'});
                modeling.editCell('rule1', 'input1', '< 1000');
                modeling.editCell('rule2', 'input1', '(2000, 5000)');
                modeling.editCell('rule3', 'input1', '> 2000');
                var ruleOrder = verifier.getInputIds();
                var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
                var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
                ...
                var overlappingRuleIDs = Object.keys(overlappingRules[0]);
                expect(overlappingRules.length === 1).to.be.true;
                expect(overlappingRuleIDs.indexOf('rule1') > -1).to.be.false;
                expect(overlappingRuleIDs.indexOf('rule2') > -1).to.be.true;
                expect(overlappingRuleIDs.indexOf('rule3') > -1).to.be.true;
                expect(overlappingRules[0].ruleValue[0] === '(2000, 5000)').to.be.true;
            }));
            
            it('if one rule finishes with another rule then returns merged value', inject(function(verifier, modeling, elementRegistry) {
                elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
                modeling.createRow({id: 'rule3'});
                modeling.editCell('rule1', 'input1', '< 1000');
                modeling.editCell('rule2', 'input1', '[200, 1000)');
                modeling.editCell('rule3', 'input1', '> 2000');
                var ruleOrder = verifier.getInputIds();
                var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
                var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
                ...
                expect(overlappingRuleIDs.indexOf('rule2') > -1).to.be.true;
                expect(overlappingRuleIDs.indexOf('rule3') > -1).to.be.false;
                expect(overlappingRules[0].ruleValue[0] === '[200, 1000)').to.be.true;
            }));
        });
        
        describe('between two intervals', function(verifier) {
            it('if one rule overlaps with another rule then returns merged value', inject(function(verifier, modeling, elementRegistry) {
                elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
                modeling.createRow({id: 'rule3'});
                modeling.editCell('rule1', 'input1', '[10, 300]');
                modeling.editCell('rule2', 'input1', '[100, 500)');
                modeling.editCell('rule3', 'input1', '[1000, 5000)');
                var ruleOrder = verifier.getInputIds();
                var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
                var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
                var rules = inputAndRules[1];
                ...
                expect(overlappingRuleIDs.indexOf('rule2') > -1).to.be.true;
                expect(overlappingRuleIDs.indexOf('rule3') > -1).to.be.false;
                expect(overlappingRules[0].ruleValue[0] === '[100, 300]').to.be.true;
            }));
            
            it('if one rule during another rule then returns merged value', inject(function(verifier, modeling, elementRegistry) {
                elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
                modeling.createRow({id: 'rule3'});
                modeling.editCell('rule1', 'input1', '[10, 50)');
                modeling.editCell('rule2', 'input1', '[50, 500]');
                modeling.editCell('rule3', 'input1', '[250, 400]');
                var ruleOrder = verifier.getInputIds();
                var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
                var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
                var rules = inputAndRules[1];
                ...
                expect(overlappingRuleIDs.indexOf('rule2') > -1).to.be.true;
                expect(overlappingRuleIDs.indexOf('rule3') > -1).to.be.true;
                expect(overlappingRules[0].ruleValue[0] === '[250, 400]').to.be.true;
            }));
            
            it('if one rule starts with another rule then returns merged value', inject(function(verifier, modeling, elementRegistry) {
                elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
                modeling.createRow({id: 'rule3'});
                modeling.editCell('rule1', 'input1', '(200, 300]');
                modeling.editCell('rule2', 'input1', '(200, 400]');
                modeling.editCell('rule3', 'input1', '(1000, 3000)');
                var ruleOrder = verifier.getInputIds();
                var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
                var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
                var rules = inputAndRules[1];
                ...
                expect(overlappingRuleIDs.indexOf('rule2') > -1).to.be.true;
                expect(overlappingRuleIDs.indexOf('rule3') > -1).to.be.false;
                expect(overlappingRules[0].ruleValue[0] === '(200, 300]').to.be.true;
            }));
            
            it('if one rule finishes with another rule then returns merged value', inject(function(verifier, modeling, elementRegistry) {
                elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
                modeling.createRow({id: 'rule3'});
                modeling.editCell('rule1', 'input1', '(300, 400]');
                modeling.editCell('rule2', 'input1', '(200, 400]');
                modeling.editCell('rule3', 'input1', '(1000, 3000)');
                var ruleOrder = verifier.getInputIds();
                var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
                ...
                expect(overlappingRuleIDs.indexOf('rule2') > -1).to.be.true;
                expect(overlappingRuleIDs.indexOf('rule3') > -1).to.be.false;
                expect(overlappingRules[0].ruleValue[0] === '(300, 400]').to.be.true;
            }));
        });
    });
    
    
    describe('missing rules', function(verifier) {
        
        it('return array should be empty if no missing rules', inject(function(verifier) {
            var ruleOrder = verifier.getInputIds();
            var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
            var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
            var rules = inputAndRules[1];
            ...
            var missingRules = verifier.findMissingRules(rules, ruleArray, 0, inputColumnCount, []);
            expect(missingRules.length === 0).to.be.true;
        }));
        
        it('should return missing value (inequality)', inject(function(verifier, modeling, elementRegistry) {
            elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
            modeling.editCell('rule1', 'input1', '> 100');
            modeling.editCell('rule2', 'input1', '100');
            var ruleOrder = verifier.getInputIds();
            ...
            expect(missingRules[0].ruleValue[0] === '< 100').to.be.true;
            
        }));
        
        it('should return missing value (inrervals)', inject(function(verifier, modeling, elementRegistry) {
            elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
            modeling.editCell('rule1', 'input1', '[20, 100]');
            modeling.editCell('rule2', 'input1', '[100, 500]');
            var ruleOrder = verifier.getInputIds();
            var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
            var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
            ...
            expect(missingRules[0].ruleValue[0] === '< 20').to.be.true;
            expect(missingRules[1].ruleValue[0] === '> 500').to.be.true;
            
        }));
    });
    
    it('should invert rules into new object if input type is string', inject(function(verifier) {
        
        var ruleOrder = verifier.getInputIds();
        var cellValues = verifier.makeCellsWithNewAttributes(ruleOrder);
        var inputAndRules = verifier.getRulesAndInputs(cellValues[0], ruleOrder);
        var rules = inputAndRules[1];
        var invertedRules = verifier.invertRules(rules, 0);
        
        expect(invertedRules['rule1'][0].value === '"gold"').to.be.true;
        expect(invertedRules['rule2'][0].value === '"silver"').to.be.true;
    }));
    
    it('should invert rules into new object if input type is numeric (inequalities)', inject(function(verifier, modeling, elementRegistry) {
        elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
        modeling.editCell('rule1', 'input1', '>= 100');
        modeling.editCell('rule2', 'input1', '< 1000');
        ...
        expect(invertedRules['rule1-<100'][0].intervalValues[0] === -Infinity).to.be.true;
        expect(invertedRules['rule1-<100'][0].intervalValues[1] === 100).to.be.true;
        expect(invertedRules['rule2->1000'][0].intervalValues[0] === 1000).to.be.true;
        expect(invertedRules['rule2->1000'][0].intervalValues[1] === Infinity).to.be.true;
        
    }));
    
    it('should invert rules into new object if input type is numeric (intervals)', inject(function(verifier, modeling, elementRegistry) {
        elementRegistry.get('input1').businessObject.inputExpression.typeRef = 'integer';
        modeling.editCell('rule1', 'input1', '[100, 1000]');
        modeling.editCell('rule2', 'input1', '(1000, 2000)');
        ...
        expect(invertedRules['rule1-<100'][0].intervalValues[0] === -Infinity).to.be.true;
        expect(invertedRules['rule1-<100'][0].intervalValues[1] === 100).to.be.true;
        expect(invertedRules['rule1->1000'][0].intervalValues[0] === 1000).to.be.true;
        expect(invertedRules['rule1->1000'][0].intervalValues[1] === Infinity).to.be.true;
